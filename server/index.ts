...
router.post("/checkBalanceAndRedirect", async (req: Request, res: Response) => {
  try {
    const { paymentAmount } = req.body; // Amount user expects to be added to the balance
    const initialBalance: string = req.session.initialBalance ?? "";
    ...
    // Check if the balance has increased by the expected amount
    if (
      parseInt(newBalance) ===
